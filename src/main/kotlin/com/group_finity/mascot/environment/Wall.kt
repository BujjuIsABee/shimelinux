...
class Wall(val area: Area, val isRight: Boolean) : Border {
    ...
    val bottom: Int
        get() = area.bottom
    val dx: Int
        @JvmName("getDX") get() = if (isRight) area.dright else area.dleft
    val dtop: Int
        @JvmName("getDTop") get() = area.dtop
    val dbottom: Int
        @JvmName("getDBottom") get() = area.dbottom
    val height: Int
        get() = area.height

    override fun isOn(location: Point): Boolean {
        return area.isVisible && (x == location.x) && (top <= location.y) && (location.y <= bottom)
    }
    ...
    override fun move(location: Point): Point {
        ...
        if ((abs(newLocation.x - location.x) >= 80) ||
            (abs(newLocation.y - location.y) >= 80)
        ) {
            return location
        }
