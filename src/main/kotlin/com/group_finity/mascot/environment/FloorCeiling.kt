...
class FloorCeiling(val area: Area, val isBottom: Boolean) : Border {
    ...
    val right: Int
        get() = area.right
    val dy: Int
        @JvmName("getDY") get() = if (isBottom) area.dbottom else area.dtop
    val dleft: Int
        @JvmName("getDLeft") get() = area.dleft
    val dright: Int
        @JvmName("getDRight") get() = area.dright
    val width: Int
        get() = area.width

    override fun isOn(location: Point): Boolean {
        return area.isVisible && (y == location.y) && (left <= location.x) && (location.x <= right)
    }
